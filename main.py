...
class Aplicacion(ctk.CTk):
    def __init__(self):
        ...
        if resultado_carga.status not in ("not_found", "ok"):
    ...
    def _construir_barra_menu(self):
        self.frame_menu = ctk.CTkFrame(self, height =44, corner_radius = 0)
    ...
    def _popup_simulado(self, opciones):
        ...
        for etiqueta, comando_real in opciones:
            menu.add_command(label=etiqueta, command=comando_real or self._mostrar_aviso)
        return menu
    ...
    def _mostrar_aviso(self):
        messagebox.showinfo(t(self.idioma, "simulado_titulo"), t(self.idioma, "simulado_msg"))
    ...
    def _abrir_settings(self):
        self._settings_win = SettingsWindow(self, self.config, on_guardar=self._guardar)

    def _guardar(self, nuevo_config, ventana):
        resultado = Config.save_config(nuevo_config)
        ...
        if resultado.ok:
            ...
            messagebox.showinfo(t(self.idioma, "aviso_titulo"),
                                t(self.idioma, "settings_guardado_ok"))
            ventana.destroy()
        else:
            # Por si no guarda
    ...
    def _reconstruir_textos_menu(self):
    ...
    def _actualizar_resumen(self):
        ...
        self.lbl_bienvenida.configure(text=t(self.idioma, "bienvenida", nombre=nombre))
    ...
    def _cargar_avatar(self):
        ...
        if ruta and os.path.isfile(ruta) and PIL_DISPONIBLE:
            try:
                ...
                self._imagen_perfil_ctk = ctk.CTkImage(light_image=img, dark_image=img, size=img.size)
