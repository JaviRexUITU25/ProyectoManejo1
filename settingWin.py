...
class SettingsWindow(ctk.CTkToplevel):
    def __init__(self, master, config: dict, on_guardar):
        ...
        self.title(t(self.idioma, "settings_title"))
        self.geometry("480x660")
        self.resizable(False, False)
        self.transient(master)
        self.grab_set()

        self._color_menu_actual = self.config_actual.get("color_menu", "#2C3E50")
    ...
    def _construir_ui(self):
        ...
        frame_fuente = ctk.CTkFrame(contenedor, fg_color="transparent")
        frame_fuente.pack(fill="x", padx=22, pady=(0, 8))
        self.entry_fuente = ctk.CTkEntry(frame_fuente, justify="center")
    ...
    def _elegir_color_menu(self):
        color = colorchooser.askcolor(color=self._color_menu_actual,
                                      title=t(self.idioma, "settings_color_menu"))
        if color and color[1]:
            self._color_menu_actual = color[1]
            self.preview_menu.configure(fg_color=self._color_menu_actual)
            
    def _elegir_color_letra(self):
        color = colorchooser.askcolor(color=self._color_letra_actual,
                                      title=t(self.idioma, "settings_color_letra"))
        if color and color[1]:
    ...
    def _elegir_foto(self):
        ruta = filedialog.askopenfilename(
            title=t(self.idioma, "settings_elegir_foto"),
